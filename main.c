...
void parse_arguments(const int argc, char * const *argv) {
    ...
    if (client_port == 0) {
        fprintf(stderr, "%s", "NO PORT DEFINED, USING DEFAULT 1143");
        client_port = 1143; // Will figure out what port I got from get socket binding.
    }

    if (is_server == true and is_client == true) {
        fprintf(stderr, "%s", "THE OUTPUTS OF BOTH SERVER AND CLIENT WILL BE ON SAME FILE!!");
    }
    if (!connection_type) {
        fprintf(stderr, "%s", "NO CONNECTION DEFINED, USING DEFAULT TCP");
        connection_type = strdup("tcp");
    }
    if (!server_hostname) {
        fprintf(stderr, "%s", "NO HOSTNAME DEFINED, USING DEFAULT 127.0.0.1");
...
void IndexGet_handler(char **argument_list, int len, int sock, int connection, struct sockaddr_in server_addr) {
    ...
    if(len >= 2) {
        ...
        else if (strcmp(argument_list[1], "--regex") == 0) 
        {
            ...
            if (reti) {
                logger_error_server("COULD NOT COMPILE REGEX");
            ...
            logger_error_server("INCORRECT FORMAT. FLAGS ALLOWED ARE shortlist, longlist, regex");
            return;
        }
    }
    ...
    if (fd == -1)
        logger_error_server("COULD NOT OPEN SHARE DIRECTORY");
    ...
    while(1) 
    {
        ...
        if (nread == -1)
            logger_error_server("GETDENTS ERROR");
        ...
        {	
            ...
            if(!hidden)
            {
                ...
                {
                    logger_error_server("Do not have search permissions in directory leading to destination");
                    return;
                }
    ...
    if (len == 2 && strcmp(argument_list[1], "--longlist") == 0) {
        ...
        logger_info_server("INDEXGET --longlist REQUEST SERVED");
    }
        ...
        logger_info_server("INDEXGET --regex REQUEST SERVED");
    }
...
void FileUpload_handler(char **argument_list, int len, int sock, int connection, struct sockaddr_in server_addr) {
    if (len < 2) {
        logger_error_server("FILENAME NOT SPECIFIED");		
        return;
    }
    if (len > 2) {
        logger_warn_server("MORE THAN ONE ARGUMENT; CHOOSING FIRST ONLY");
    }
...
void FileDownload_handler(char **argument_list, int len, int sock, int connection, struct sockaddr_in server_addr) {
    ...
    if(access(filename, R_OK) != 0) {
        logger_error_server("UNABLE TO ACCESS FILE");
        return;
    } else {
        char c;
